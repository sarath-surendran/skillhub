...
const ChatComponent = ({reciever_id, course_id}) => {
  ...
  const fetchReceiverDetails = async () => {
    try{
      ...
      console.error("error getting receiver details",error)
    }
  }
  ...
  useEffect(() => {
    ...
    //   //   let received_message = received_data.message
        // if (!chatLog.some((msg) => msg.id === received_message.id)) {
        //   setChatLog((prevChatLog) => [...prevChatLog, received_message]);
        // }
    //   // }
    //   //   setChatLog([...chatLog, received_message])
    //     const receivedData = JSON.parse(event.data);
    //     if (receivedData.type === 'message') {
    //       // Handle regular messages
    //       setChatLog((prevChatLog) => [...prevChatLog, receivedData]);
    //     } else if (receivedData.type === 'typing') {
    //       // Handle typing indicators
    //       setIsTyping(receivedData.isTyping);
    //     }
    ...
    socket.onmessage = (event) => {
      ...
      if (receivedData.type === 'message') {
        ...
        if(receivedData.user_id != user.id){
          setIsTyping(receivedData)
        }
      }
    };
  ...
  }, [reciever_id, course_id]);

  console.log(chatLog);
  console.log("typing status", isTyping);
  const sendMessage = () => {
    if (message && ws && ws.readyState === WebSocket.OPEN) {
      const newMessage = { "message":message, type:"message" };
  ...
  const sendTypingStatus = (typing) => {
    if(ws && ws.readyState === WebSocket.OPEN){
      console.log(typing)
      const typingData = { type : "typing", "isTyping":typing}
          ...
          <div class="relative flex items-center space-x-4">
            ...
            <div class="flex flex-col leading-tight">
              <div class="text-2xl mt-1 flex items-center">
                <span class="text-gray-700 mr-3">{receiverDetails.name}</span>
              </div>
              {/* <span class="text-lg text-gray-600">Junior Developer</span> */}
              {isTyping.is_typing && isTyping.reciever_id == user.id && <span className='text-lg text-gray-600'>typing...</span>}
            </div>
          </div>
